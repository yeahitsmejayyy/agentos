...
It reads the mind (the `.mem` vault — sessions, decisions, what recurs) and the shape of the work. Four
...
orchestrator owns at the task altitude, raised to the system-evolution altitude. Drift proposals are how
...
|---|---|
| The loop, the four signal classes, the §4 gate, the bar | The recall-side "N proposals waiting" surface in `recall-orient.sh` |
