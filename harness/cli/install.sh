...
chmod +x "$OS/harness/cli/agentos"
ok "agentos entrypoint ready"

echo
echo "✓ Generic CLI adapter installed. The Mind (${HOME_DIR}) is shared with any other adapter — unchanged."
echo "  Boot:     $OS/harness/cli/agentos boot"
echo "  On PATH:  ln -s $OS/harness/cli/agentos ~/.local/bin/agentos   # then just: agentos boot"
